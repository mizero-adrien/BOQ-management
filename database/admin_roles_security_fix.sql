-- =================================================================
-- SECURITY FIX for admin_roles.sql
--
-- admin_roles.sql had two critical holes:
-- 1. Postgres grants EXECUTE to PUBLIC by default on new functions,
--    which was never revoked — so every admin RPC was callable by
--    anyone with just the public anon key, no login required.
-- 2. grant_admin_access / revoke_admin_access / suspend_user /
--    unsuspend_user / suspend_company / unsuspend_company trusted a
--    client-supplied "*_by_id" parameter to decide who the caller is,
--    instead of auth.uid(). Combined with (1), anyone could pass a
--    known super_admin's id (discoverable via the same leak) and
...
-- =================================================================
-- STEP 1 — Lock down EXECUTE to authenticated only
-- =================================================================

revoke execute on function is_platform_admin() from public;
revoke execute on function is_super_admin() from public;
revoke execute on function grant_admin_access(uuid, text, uuid) from public;
revoke execute on function revoke_admin_access(uuid, uuid) from public;
revoke execute on function suspend_user(uuid, text, uuid) from public;
revoke execute on function unsuspend_user(uuid, uuid) from public;
revoke execute on function suspend_company(uuid, text, uuid) from public;
revoke execute on function unsuspend_company(uuid, uuid) from public;
revoke execute on function get_admin_companies() from public;
revoke execute on function get_admin_users(text, text, text, int, int) from public;
revoke execute on function get_platform_stats(int) from public;
...
-- =================================================================
-- STEP 2 — Re-derive the caller from auth.uid(), never from a
-- client-supplied parameter. The "*_by_id" parameters are kept in
-- the signature (so the existing frontend calls don't need to
-- change) but their value is no longer trusted for anything.
-- =================================================================
...
begin
  ...
  insert into admin_audit_log (admin_id, action, target_type, target_id, details)
  values (v_caller_id, 'suspend_company', 'company', target_company_id, jsonb_build_object('reason', reason));
end;
$$;
...
begin
  ...
  insert into admin_audit_log (admin_id, action, target_type, target_id, details)
  values (v_caller_id, 'unsuspend_company', 'company', target_company_id, null);
end;
$$;


-- =================================================================
-- STEP 3 — Add a caller-is-admin guard to the read RPCs. Being
-- restricted to `authenticated` isn't enough on its own — any
-- logged-in non-admin user could otherwise still call these and
-- read every user/company's data.
-- =================================================================
...
-- =================================================================
-- STEP 4 — platform_settings had the same PUBLIC-grant gap via its
-- RLS policies being fine, but double check EXECUTE isn't the issue
-- here (it's a table, governed by RLS only, not a function) — no
-- action needed, included for completeness of the audit.
-- =================================================================
