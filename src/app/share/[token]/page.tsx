...
export default async function SharePage({ params }: Props) {
  ...
  const [zonesRes, reportsRes, boqSectionsRes, tasksRes] = await Promise.all([
    ...
    supabase.from('boq_sections').select('id, title, order_index').eq('project_id', project.id).order('order_index'),
  ...
  const rawSections = boqSectionsRes.data ?? []
  const sectionItems = rawSections.length > 0
    ? await supabase.from('boq_items').select('section_id, budgeted_total, used_total').in('section_id', rawSections.map((s) => s.id as string))
    : { data: [] }
  const itemRows = sectionItems.data ?? []

  const sections = rawSections.map((s) => {
    const its = itemRows.filter((i) => i.section_id === s.id)
    return {
      id: s.id as string,
      title: s.title as string,
      total_budgeted: its.reduce((sum, i) => sum + (Number(i.budgeted_total) || 0), 0),
      total_used: its.reduce((sum, i) => sum + (Number(i.used_total) || 0), 0),
