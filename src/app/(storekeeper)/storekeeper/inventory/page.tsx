...
function statusBadge(item: BOQItem) {
  const pct = item.budgeted_total > 0 ? (item.used_total / item.budgeted_total) : 0
  if (pct >= 1) return { label: 'Over budget', bg: '#E24B4A', color: '#FFFFFF', border: false }
...
function InventoryRow({ item, isLast }: { item: BOQItem; isLast: boolean }) {
  const usedQty = item.unit_rate > 0 ? item.used_total / item.unit_rate : 0
  ...
  return (
    <div className={`grid gap-2 px-4 py-3 text-xs ${isLast ? '' : 'border-b'}`}
      style={{ borderColor: '#EEEEEE', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr' }}>
      ...
      <p style={{ color: '#666666' }}>{usedQty.toFixed(1)}</p>
    ...
    <div className="px-4 py-5 md:px-8 md:py-8" style={{ maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div className="bg-white rounded-xl overflow-hidden" style={{ border: '0.5px solid #EEEEEE' }}>
        <div className="grid gap-2 px-4 py-2 border-b text-xs font-semibold uppercase tracking-wider"
          style={{ borderColor: '#EEEEEE', color: '#BBBBBB', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr' }}>
          <span>Item</span><span>Unit</span><span>Budget Qty</span><span>Used Qty</span><span>Remaining</span><span>Budget</span><span>Used</span><span>Status</span>
        </div>
