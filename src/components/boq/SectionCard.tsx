      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          ...
          <p className="text-sm font-semibold mt-0.5" style={{ color: '#111111' }}>
            {formatCurrency(section.total_budgeted)}
          </p>
        </div>
        <div>
          <p className="text-xs" style={{ color: '#666666' }}>
          ...
          <p className="text-sm font-semibold mt-0.5" style={{ color: '#111111' }}>
            {formatCurrency(section.total_used)}
          </p>
        </div>
      </div>

      <BudgetProgressBar
        used={section.total_used}
        total={section.total_budgeted}
      />

      {section.usage_pct >= 80 && (
        <p
          className="text-xs font-medium mt-1.5"
          style={{ color: '#E24B4A' }}
        >
          {section.usage_pct >= 100
            ? 'Over budget'
            : `${section.usage_pct}% used — near limit`}
        </p>
      )}
    </Link>
  )
