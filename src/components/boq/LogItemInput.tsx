...
export default function LogItemInput({
  ...
  const cost = hasFinancials ? quantity * item.unit_rate! : null
  const wouldExceed = hasFinancials && cost !== null
    ? (item.used_total ?? 0) + cost > (item.budgeted_total ?? 0)
    : false
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0 mr-3">
          ...
          <p className="text-xs mt-0.5" style={{ color: '#666666' }}>
            Unit: {item.unit}
            {hasFinancials && ` · Rate: ${item.unit_rate!.toLocaleString()} RWF`}
          </p>
        </div>
      </div>

      <div className="flex items-center gap-3">
        <button type="button" onClick={() => quantity > 0 && onChange(quantity - 1)}
          disabled={quantity <= 0}
        ...
        <span className="text-lg font-semibold w-12 text-center" style={{ color: '#111111' }}>{quantity}</span>
        ...
        {hasFinancials && cost !== null && (
          <div className="ml-auto text-right">
            <p className="text-xs" style={{ color: '#666666' }}>Cost today</p>
            <p className="text-sm font-semibold" style={{ color: '#111111' }}>{cost.toLocaleString()} RWF</p>
          </div>
        )}
      </div>

      {wouldExceed && quantity > 0 && (
        <p className="text-xs font-medium mt-2" style={{ color: '#E24B4A' }}>
