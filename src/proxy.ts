...
export async function proxy(request: NextRequest) {
  ...
  // getUser() makes a Supabase API call on every request and can time out
  // right after a fresh OAuth session is established, returning null and
  // causing a spurious redirect to /login.
  const { data: { session } } = await supabase.auth.getSession()
  const user = session?.user ?? null

  console.log('[proxy] ' + pathname + ' ' + JSON.stringify({
